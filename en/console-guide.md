...
2. 장애조치를 위해 Master 노드의 다운 여부를 판단하기 위한 헬스체크 응답대기시간을 설정할 수 있습니다. 기본값은 3000ms 입니다.
...
- 마스터 다운판정시간 : 장애조치를 위해 마스터 노드의 다운 여부를 판단하기 위한 헬스체크 응답대기시간을 변경할 수 있습니다. 기본값은 3000ms 이며, Replica 노드가 있을 경우에만 변경할 수 있는 항목입니다.
- 자동 백업 설정: 자동 백업 사용 여부를 선택합니다.
	- 백업 보관 기간 : 1일부터 최대 30일까지 보관할 수 있습니다.
...
- 플로팅 IP를 연결하지 않은 Redis 노드는 외부에서 접근할 수 없습니다.
- 플로팅 IP를 연결할 경우에는 공인 도메인이 발행됩니다.
...
생성된 복제 그룹을 선택하고 **노드 정보** 탭을 누르면 복제 그룹 노드의 상세 정보 확인과 Replica 노드의 Master 노드 승격을 할 수 있습니다.
...
EasyCache는 Redis 운영 및 사용에 필요한 모니터링 항목을 주기적으로 수집하고 차트로 보여줍니다. 특정 복제 그룹의 모니터링 항목이 보고 싶다면, 복제 그룹 목록에서 원하는 복제 그룹을 선택하고 **모니터링** 탭을 클릭합니다.

![monitoring_001.png](https://static.toastoven.net/prod_easycache/20.04.28/rep_monitoring_001.PNG)
...
![filter_001.PNG](https://static.toastoven.net/prod_easycache/20.04.28/rep_filter_002.PNG)
- 모니터링 데이터 보존 기간은 1개월입니다.
- 모니터링 항목은 **필터 조건**에서 원하는 항목만을 표시하도록 선택할 수 있습니다.
- 모니터링 항목은 다음과 같습니다.
  - CPU 이용률, 시스템 메모리, 연결된 클라이언트, 블록된 클라이언트
  - Redis 메모리 사용량, Redis 메모리 사용량(rss), 메모리 파편화 비율
  - 초당 처리한 명령 수, 입력 바이트, 출력 바이트, 만료된 키 수(expired), 삭제된 키 수(evicted)
  - 조회 성공 수, 조회 실패 수, 조회 성공률, 키 개수
  - get 실행 횟수, get usec/get calls, set 실행 횟수, set usec/get calls
